...
- **TUI status polling gates must include all recoverable states** — The TUI's `updateOutputs` loop gates `updateInstanceStatus` calls on instance status. If a new status is added (e.g., `StatusStuck`) but not included in the gate, the TUI will never poll that instance again, even if it recovers. Similarly, `updateInstanceStatus` must handle transitions *from* every polled status, not just the original ones. When adding a new instance status, check both gates.
