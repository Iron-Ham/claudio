...
- **`startJudge` must use `attemptSuccess`, not `session.Attempts[i].Status`** — `TeamCompletedEvent` fires from `monitorTeamCompletion` (triggered by `queue.depth_changed` from `gate.Complete()`), while `session.Attempts[i].Status` is set by `onBridgeTaskCompleted` (triggered by `bridge.task_completed`, published AFTER `gate.Complete()` returns). On loaded machines, `startJudge` runs before `onBridgeTaskCompleted` updates the session, so `SuccessfulAttemptCount()` returns 0. The `attemptSuccess[3]bool` field, populated from `TeamCompletedEvent.Success` in `onTeamCompleted`, is always set before `startJudge` is dispatched.
