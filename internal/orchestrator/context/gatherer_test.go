...
func TestNewGatherer_NilFinder(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Error("expected panic when finder is nil")
		}
	}()
	reader := newMockCompletionFileReader()
	NewGatherer(nil, reader)
}

func TestNewGatherer_NilReader(t *testing.T) {
	defer func() {
		if r := recover(); r == nil {
			t.Error("expected panic when reader is nil")
		}
	}()
	finder := newMockInstanceFinder()
	NewGatherer(finder, nil)
}
