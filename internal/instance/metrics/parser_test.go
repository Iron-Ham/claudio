...
func TestMetricsParser_Parse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			metrics, err := parser.Parse([]byte(tt.output))
			if err != nil {
...
func TestParsedMetrics_CacheFields(t *testing.T) {
	parser := NewMetricsParser()

	// Test that cache fields are correctly parsed
	output := "Cache: 1.2K read, 500 write"
	metrics, err := parser.Parse([]byte(output))
	if err != nil {
		t.Fatalf("Parse() returned unexpected error: %v", err)
	}

	if metrics == nil {
		t.Fatal("Parse() returned nil, want metrics")
	}

	if metrics.CacheReadTokens != 1200 {
		t.Errorf("CacheReadTokens = %d, want 1200", metrics.CacheReadTokens)
	}
	if metrics.CacheWriteTokens != 500 {
		t.Errorf("CacheWriteTokens = %d, want 500", metrics.CacheWriteTokens)
	}
}
