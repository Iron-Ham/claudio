...
func TestEnterTerminalMode(t *testing.T) {
	// Test the case where terminal is not visible - enterTerminalMode should be a no-op
	t.Run("does not enter when terminal not visible", func(t *testing.T) {
		m := Model{
			terminalVisible: false,
			terminalMode:    false,
		}
		...
		if m.terminalMode != false {
			t.Error("terminalMode should remain false when terminal is not visible")
		}
	})

	// Note: Testing "does not enter when no terminal process" would require
	// mocking the terminal process interface, which is out of scope for this change.
	// The enterTerminalMode method checks terminalProcess != nil && terminalProcess.IsRunning()
	// before setting terminalMode = true.
}
