...
func TestOutputHeightCalculation(t *testing.T) {
	...
	// The formula should be: outputHeight = max(totalHeight - 3, 3)
	// Where 3 is subtracted for: 2 border lines + 1 header line
	// And minimum output height is 3 (consistent with tmux content area minimum)
	tests := []struct {
		name              string
		totalHeight       int
		...
		{
			name:              "height 6 gives 3 output lines",
			totalHeight:       6,
			expectedLineCount: 3, // 6 - 3 = 3
			outputLines:       5,
			expectTruncation:  true,
		},
		{
			name:              "minimum height clamps to 3 output lines",
			totalHeight:       4,
			expectedLineCount: 3, // max(4 - 3, 3) = 3
			outputLines:       5,
			expectTruncation:  true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Calculate expected output height (matches Render logic)
