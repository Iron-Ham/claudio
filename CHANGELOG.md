...
- **PromptAdapter Infrastructure** - Added `PromptAdapter` struct and converter methods to bridge orchestrator types (`PlanSpec`, `PlannedTask`) to `prompt.Context` types, enabling the existing prompt.Builder infrastructure to be used for prompt generation. Includes `BuildTaskContext` method that creates task execution prompts with plan info, task details, group index, and previous group context.
