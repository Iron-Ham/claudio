...
- **System Prompt Separation** - Extracted orchestration instructions (guidelines + completion protocol) from inline task prompts into a dedicated system prompt file injected via Claude Code's `--append-system-prompt-file` flag. The `PipelineRunner` automatically writes `WriteOrchestrationSystemPrompt` to the pipeline's base directory and injects it into execution role overrides. Bridge `BuildTaskPrompt` now includes the task ID so instances can reference it in the completion file. System prompt content persists across the entire session, keeping task prompts focused on the specific task.

- **Per-Role CLI Flag Overrides** - Wired role-specific `StartOptions` overrides from pipeline configuration through the full instance creation chain (`PipelineExecutor → Bridge → InstanceFactory → Orchestrator → Manager → Backend`). Different pipeline roles (execution, review, planning) can now use different Claude Code CLI flags (permission mode, model, max turns, tool restrictions). Added `RoleOverrides` to `PipelineRunnerConfig`/`PipelineExecutorConfig`, `NewInstanceFactoryWithOverrides` for per-team factory creation, and `StartInstanceWithOverrides` on the Orchestrator.

- **Claude Code Advanced CLI Integration** - Expanded `ClaudeBackendConfig` and `StartOptions` to support modern Claude Code CLI features: granular permission modes (`default`, `plan`, `auto-accept`, `bypass`) replacing the binary `skip_permissions` flag; per-tool `allowed_tools`/`disallowed_tools` restrictions; `max_turns` for bounding agentic work; `model` selection per-session or per-invocation; `append_system_prompt` for injecting orchestration context without replacing the default prompt; `--no-user-prompt` for headless pipelines; and `--output-format` support (`text`, `json`, `stream-json`) for structured output. All new options support per-invocation overrides from `StartOptions`, enabling the bridge/orchestrator to customize behavior per pipeline role (e.g., `plan` mode for reviewers, `bypass` for implementers).
...
- **False Stale Timeout for Idle Instances** - Fixed stale detection incorrectly marking idle instances (waiting for user input at the `❯` prompt) as stuck. The stale counter now skips incrementing when the state detector identifies the instance as waiting for input/question/permission. Also resets the stale counter when an instance is resumed (tab switch), preventing accumulated ticks across pause/resume cycles from triggering false positives. Added `ResumeActiveInstance` to TUI update context so `HandleInstanceSetupComplete` error paths don't leave the previously-active instance permanently paused.
